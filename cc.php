...
class ClashCaller
{
    ...
    public function format_calls($data, $only_active = false)
    {
        ...
        foreach ($calls as $num => $call) {
            ...
            array_push($out, sprintf('#%d: %s', $num + 1, implode(', ', $call__)));
        }
        if (count($out) == 1) {
    ...
    public function format_call($data, $general, $status = false)
    {
        ...
        return array('active' => $active_call, 'name' => $data['playername'], 'timer' => $timer_text, 'x' => $data['posx']);
    }
        ...
        foreach ($data as $call) {
            ...
            if ($push_call) {
                ...
                if (!isset($calls[$y_][$x_])) {
                    $calls[$y_][$x_] = $this->format_call($call, $general_);
                }
            }
        }
        ksort($calls);
